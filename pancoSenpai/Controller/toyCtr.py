...
from pancoSenpai.View.resView import ResView
...
class ToyCtr:
    ...
    def appliquer_modifs_server(self):
        ...
        self.dessiner()
        text_s, text_f = self.graph.afficher_reseau(0)
    ...
    def analyser(self):
        tfa = self.view.get_tfa()
        sfa = self.view.get_tfa()
        plp = self.view.get_plp()
        elp = self.view.get_elp()
        network, methods, results_delays = self.graph.analyze_fifo(tfa, sfa, plp, elp)
        ResView(self.view, network, methods, results_delays)
    ...
    def save(self):
        ...
        plp = self.view.get_plp()
        elp = self.view.get_elp()
        self.graph.write_file(tfa, sfa, plp, elp, "toy.py")
